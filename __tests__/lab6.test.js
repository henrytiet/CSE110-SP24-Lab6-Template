describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const exampleElement = await page.$('product-item');
    // Grab the shadowRoot of that element (it's a property), then query a button from that shadowRoot.
    const shadowOf = await exampleElement.getProperty('shadowRoot');

    const butn = await shadowOf.asElement().$('button');
    // Once you have the button, you can click it and check the innerText property of the button.
    await butn.click();
    const inside = await butn.getProperty('innerText');
    const txt = await inside.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i=1; i < prodItems.length; i++) {
      const shadowOf = await prodItems[i].getProperty('shadowRoot');
      const butn = await shadowOf.asElement().$('button');
      await butn.click();
    }    
    // Check to see if the innerText of #cart-count is 20
    const cart = await page.$('#cart-count');
    const inner = await cart.getProperty('innerText');
    const txt = await inner.jsonValue();
    expect(txt).toBe('20');
  }, 100000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i=0; i < prodItems.length; i++) {
      ...
      const butn = await shadowOf.asElement().$('button');
      const inside = await butn.getProperty('innerText');
      const btntxt = await inside.jsonValue();
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const cart = await page.evaluate(() => {
      return cart = window.localStorage.getItem('cart'); 
    });
